...
type fileTtsQwen3TTS struct {
	...
	Voice       string  `toml:"voice"`       // voice name (default: Vivian)
	Speed       float64 `toml:"speed"`       // 0.5–2.0; 0 means use default (1.0)
...
//  2. /opt/clawproxy/config.toml (default — service WorkingDirectory)
//  3. ~/.clawproxy/config.toml  (fallback — alongside saved tokens)
