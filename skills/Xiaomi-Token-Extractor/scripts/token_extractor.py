...
parser.add_argument("--retries", required=False, help="Times to re-generate QR if it expires before scan (default: 2)", type=int, default=2)
...
class PasswordXiaomiCloudConnector(XiaomiCloudConnector):
    ...
    def handle_captcha(self, captcha_url: str) -> str:
        ...
        present_image_image(
            response.content,
            message_url = f"Image URL: {Fore.BLUE}http://{_get_local_ip()}:31415",
...
class QrCodeXiaomiCloudConnector(XiaomiCloudConnector):
    ...
    def login(self) -> bool:
        ...
        for attempt in range(1, max_attempts + 1):
            if attempt > 1:
                print_if_interactive(f"{Fore.YELLOW}QR code expired, generating a new one... (attempt {attempt}/{max_attempts})")
            ...
            result = self.login_step_3()  # None = expired/timeout, False = hard error
            if result is None:
                continue   # regenerate QR
            if not result:
        ...
        print_if_interactive(f"{Fore.RED}QR code expired on all {max_attempts} attempt(s). Please run again.")
        return False
    ...
    def login_step_2(self) -> bool:
        ...
        if valid:
            ...
            present_image_image(
                response.content,
                message_url = f"QR code URL: {Fore.BLUE}http://{_get_local_ip()}:31415",
    ...
    def login_step_3(self) -> bool | None:
        """Long-poll until user scans the QR code.

        Returns:
            True  — scanned successfully.
            None  — QR expired / timed out (caller can regenerate).
            False — hard network error.
        """
        _LOGGER.debug("login_step_3")
        ...
        while True:
            try:
                ...
                if time.time() - start_time > self._timeout:
                    ...
                    return None   # retriable — caller regenerates QR
                continue
            ...
            _LOGGER.debug("Long polling: status %s, retrying...", response.status_code)
            time.sleep(1)

        if response is None or response.status_code != 200:
            _LOGGER.error("Long polling failed.")
            return None
        ...
        _LOGGER.debug("Location: " + str(self._location))
...
def start_image_server(image: bytes) -> None:
    ...
    httpd = HTTPServer(("0.0.0.0", 31415), ImgHttpHandler)
...
def _get_local_ip() -> str:
    """Return the LAN IP that other devices can reach this machine on.

    Precedence:
      1. --host CLI override
      2. UDP routing trick (no packets sent; kernel fills in source IP)
      3. hostname → address resolution
      4. Enumerate all non-loopback AF_INET addresses
    Falls back to 127.0.0.1 only if every strategy fails.
    """
    if args.host:
        return args.host
